...
import { Plus, Pencil, Trash2, BookOpen, FileText, Upload, X, Loader2 } from 'lucide-react';
...
export default function AdminPublicationsPage() {
  ...
  const fetchPublications = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/publications?admin=true');
      const data = await res.json();
      setPublications(data.publications ?? data ?? []);
    } catch {
      setGlobalError('Failed to load publications');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  function openEdit(pub: Publication) {
    setEditingId(pub.id);
    setEditCoverImage(pub.coverImage ?? '');
    setEditPdfFile(pub.pdfFile ?? '');
    setEditYear(pub.year ? String(pub.year) : '');
    setEditPublished(pub.published);
    setError('');
    setSuccess('');
    setSheetOpen(true);
  }
  ...
  async function handleAddSubmit(e: React.FormEvent) {
    ...
    if (ready.length === 0) { setError('Upload at least one cover image first.'); return; }
      ...
      const results = await Promise.all(
        ready.map((item) =>
          fetch('/api/publications', {
            ...
            body: JSON.stringify({ coverImage: item.url, year: addYear ? Number(addYear) : null, published: addPublished }),
          })
        )
      );
      if (!results.every((r) => r.ok)) { setError('Some publications failed to save.'); return; }
      setSuccess(`${ready.length} publication${ready.length > 1 ? 's' : ''} added!`);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/publications/${deleteId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ _delete: true }) });
      if (!res.ok) {
        const data = await res.json();
  ...
  return (
    <div className="p-8 max-w-[1100px]">
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div>
          <div className="text-2xl font-bold text-heading">Publications</div>
          <p className="text-slate-500 text-sm mt-0.5">{publications.length} publication{publications.length !== 1 ? 's' : ''} total</p>
        </div>
        <Button onClick={openAdd}><Plus size={15} />Add Publications</Button>
      </div>
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <Button className="mt-4" onClick={openAdd}><Plus size={15} />Add First Publication</Button>
          </div>
        ) : (
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {publications.map((pub) => (
                <TableRow key={pub.id}>
                  ...
                  <TableCell className="text-slate-500 text-sm">{pub.year ?? '—'}</TableCell>
                  <TableCell>
                    {pub.pdfFile ? (
                  ...
                  <TableCell>
                    <Badge variant={pub.published ? 'success' : 'secondary'}>
                      {pub.published ? 'Published' : 'Hidden'}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Button variant="outline" size="sm" onClick={() => openEdit(pub)}><Pencil size={13} />Edit</Button>
                      <Button variant="destructive" size="sm" onClick={() => setDeleteId(pub.id)}><Trash2 size={13} />Delete</Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent open={sheetOpen}>
          <SheetHeader>
            <SheetTitle>{editingId ? 'Edit Publication' : 'Add Publications'}</SheetTitle>
          </SheetHeader>
          <SheetBody>
            ...
            {editingId ? (
              <form id="pub-form" onSubmit={handleEditSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <Label>Cover Image</Label>
                  <ImageUpload value={editCoverImage} onChange={setEditCoverImage} previewClass="aspect-[3/4] max-h-48" />
                </div>
                <div className="space-y-1.5">
                  <Label>PDF File</Label>
                  <FileUpload value={editPdfFile} onChange={setEditPdfFile} accept=".pdf,application/pdf" />
                </div>
                <div className="space-y-1.5">
              ...
              <form id="pub-form" onSubmit={handleAddSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs mt-0.5 text-slate-300">Select multiple cover images at once</p>
                    </div>
                  </button>
                ...
                {uploadItems.length > 0 && (
                  <div className="grid grid-cols-3 gap-2">
                    {uploadItems.map((item) => (
                      <div key={item.uid} className="relative rounded-lg overflow-hidden border border-slate-200 aspect-[3/4] bg-slate-100">
          ...
          <SheetFooter>
            ...
            <Button type="submit" form="pub-form" disabled={saving || (!editingId && readyCount === 0)}>
              {saving ? 'Saving…' : editingId ? 'Update Publication' : `Add ${readyCount > 0 ? readyCount : ''} Publication${readyCount !== 1 ? 's' : ''}`}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
      ...
      <Dialog open={deleteId !== null} onOpenChange={(open) => !open && setDeleteId(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Are you sure you want to delete this publication? This action cannot be undone.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
