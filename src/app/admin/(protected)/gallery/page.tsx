...
import { Plus, Pencil, Trash2, Images, Upload, X, Loader2 } from 'lucide-react';
...
const ALBUM_OPTIONS = [
  ...
  { value: 'media-support', label: 'Media Support (/media-support)' },
];
...
export default function AdminGalleryPage() {
  ...
  const [addAlbum, setAddAlbum] = useState('gallery');
  const [addPublished, setAddPublished] = useState(true);

  // Edit mode state
  const [editImage, setEditImage] = useState('');
  const [editAlbum, setEditAlbum] = useState('gallery');
  ...
  const fetchImages = useCallback(async () => {
    ...
    try {
      ...
      setGlobalError('Failed to load gallery');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  function openEdit(img: GalleryImage) {
    setEditingId(img.id);
    setEditImage(img.image);
    setEditAlbum(img.album ?? 'gallery');
    ...
    newItems.forEach((item) => {
      uploadFile(item.file)
        ...
        .catch((err) => {
          setUploadItems((prev) =>
            prev.map((i) =>
              i.uid === item.uid ? { ...i, status: 'error', error: err.message } : i
            )
          );
        });
    });
  ...
  function removeUploadItem(uid: string) {
      ...
      const results = await Promise.all(
        ready.map((item) =>
          fetch('/api/gallery', {
            ...
            body: JSON.stringify({ image: item.url, album: addAlbum, published: addPublished }),
          })
        )
      );
      const allOk = results.every((r) => r.ok);
      if (!allOk) { setError('Some images failed to save.'); return; }
      setSuccess(`${ready.length} image${ready.length > 1 ? 's' : ''} added!`);
      await fetchImages();
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/gallery/${editingId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ image: editImage, album: editAlbum, published: editPublished }),
      });
      const data = await res.json();
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/gallery/${deleteId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ _delete: true }) });
      if (!res.ok) {
        const data = await res.json();
  ...
  return (
    <div className="p-8 max-w-[1200px]">
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div>
          <div className="text-2xl font-bold text-heading">Gallery</div>
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <Images size={40} className="text-slate-300 mx-auto mb-3" />
            <p className="text-slate-400 text-sm">No gallery images yet.</p>
            <Button className="mt-4" onClick={openAdd}><Plus size={15} />Add First Image</Button>
          </div>
        ) : (
          <Table>
            <TableHeader>
              ...
              {images.map((img) => (
                <TableRow key={img.id}>
                  ...
                  <TableCell className="text-slate-500 text-sm">
                    {img.album === 'media-support' ? 'Media Support' : 'Gallery'}
                  </TableCell>
                  <TableCell className="text-slate-500 text-sm">
                    {new Date(img.createdAt).toLocaleDateString('en-IN')}
                  </TableCell>
                  <TableCell>
                    <Badge variant={img.published ? 'success' : 'secondary'}>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent open={sheetOpen}>
          <SheetHeader>
            <SheetTitle>{editingId ? 'Edit Image' : 'Add Images'}</SheetTitle>
          </SheetHeader>
          <SheetBody>
            {error && (
            ...
            {success && (
              <div className="mb-4 px-4 py-3 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm">{success}</div>
            )}

            {editingId ? (
              /* ── Edit form ── */
              <form id="gallery-form" onSubmit={handleEditSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="edit-album">Section</Label>
                  <select
                    id="edit-album"
                    value={editAlbum}
                    onChange={(e) => setEditAlbum(e.target.value)}
                    className="w-full h-9 rounded-md border border-slate-200 bg-white px-3 text-sm text-heading focus:outline-none focus:ring-2 focus:ring-primary/40"
                  >
                ...
                <label className="flex items-center gap-2.5 cursor-pointer px-3 py-2 bg-slate-50 rounded-lg border border-slate-200 w-full">
                  <Checkbox
                    ...
                    onChange={(e) => setEditPublished((e.target as HTMLInputElement).checked)}
                  />
                  <span className="text-sm font-medium text-heading">Published</span>
                </label>
              </form>
            ) : (
              /* ── Add / multi-upload form ── */
              <form id="gallery-form" onSubmit={handleAddSubmit} className="space-y-4">
                {/* Drop zone */}
                <div className="space-y-1.5">
                  <Label>Images</Label>
                  <button
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {uploadItems.map((item) => (
                      <div key={item.uid} className="relative rounded-lg overflow-hidden border border-slate-200 aspect-square bg-slate-100">
                        ...
                        <button
                          type="button"
                          onClick={() => removeUploadItem(item.uid)}
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="add-album">Section</Label>
                  <select
                    id="add-album"
                    value={addAlbum}
                ...
                <label className="flex items-center gap-2.5 cursor-pointer px-3 py-2 bg-slate-50 rounded-lg border border-slate-200 w-full">
                  <Checkbox
                    ...
                    onChange={(e) => setAddPublished((e.target as HTMLInputElement).checked)}
                  />
                  <span className="text-sm font-medium text-heading">Published</span>
                </label>
              </form>
          ...
          <SheetFooter>
            ...
            <Button
              type="submit"
              form="gallery-form"
              disabled={saving || (!editingId && readyCount === 0)}
            >
              {saving
                ? 'Saving…'
                : editingId
                  ? 'Update Image'
                  : `Add ${readyCount > 0 ? readyCount : ''} Image${readyCount !== 1 ? 's' : ''}`}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
